...
class ProxyChecker:
    ...
    def get_info(self, url=None, proxy=None, auth=None):
        info = {}
        proxy_type = []
        ...
        if url != None:
            ...
            for protocol in ['http', 'socks4', 'socks5']:
                ...
                try:
                    start = time()
                    if auth != None:
                        response = requests.get(random.choice(judges), auth=auth, proxies=proxy_dict, headers=headers, timeout=5)
                    else:
    ...
    def check_proxy(self, proxy, auth):
        ip = proxy.split(':')
        resp = self.get_info(proxy=proxy, auth=auth)
...
async def check_one_proxy(proxy):
    ...
    if len(proxy.split(":")) == 4:
        auth = (split_p[2], split_p[3])
        proxy = f"{split_p[0]}:{split_p[1]}"
    else:
        auth=None
    checker = ProxyChecker()
    resault = checker.check_proxy(proxy, auth)
    return resault

async def check_many_proxy(file):
